function [R, A, D, t] = algorithm_HTS_PNF_WOA(para, H, user_r, user_theta, W_initial, theta_plot, r_plot, plot_flag)
%The piecewise-near-field (PNF)-based heuristic two-stage approach using WOA
%  [R, A, D, t] = algorithm_HTS_PNF_WOA(para, H, user_r, user_theta, W_initial, theta_plot, r_plot, plot_flag)
%Inputs:
...
if plot_flag && nargin >= 7
    [P_WOA_PNF, ~] = beampattern_WOA(para, theta_plot, r_plot, A, t, [], []);
    figure;
    plot(para.fm_all/1e9, 10*log10(P_WOA_PNF/para.N), 'LineWidth', 1.5);
    xlabel('Frequency (GHz)', 'Interpreter', 'Latex');
    ylabel('Array Gain (dB)', 'Interpreter', 'Latex');
    title('Array Gain for WOA PNF Method', 'Interpreter', 'Latex');
    grid on;
end
end
